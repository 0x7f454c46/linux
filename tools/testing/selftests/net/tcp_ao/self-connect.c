			     ...
			     bool different_keyids, bool check_restore)
{
	...
	struct tcp_ao_counters before_ao1, before_ao2;
	struct tcp_ao_counters after_ao1, after_ao2;
	...
	if (different_keyids) {
		if (test_get_tcp_ao_counters(sk, &before_ao1, local_addr, -1, 5, 7))
			test_error("test_get_tcp_ao_counters()");
		if (test_get_tcp_ao_counters(sk, &before_ao2, local_addr, -1, 7, 5))
			test_error("test_get_tcp_ao_counters()");
	} else {
		if (test_get_tcp_ao_counters(sk, &before_ao1, local_addr, -1, 100, 100))
			test_error("test_get_tcp_ao_counters()");
	}
	...
	if (different_keyids) {
		if (test_get_tcp_ao_counters(sk, &after_ao1, local_addr, -1, 5, 7))
			test_error("test_get_tcp_ao_counters()");
		if (test_get_tcp_ao_counters(sk, &after_ao2, local_addr, -1, 7, 5))
			test_error("test_get_tcp_ao_counters()");
	} else {
		if (test_get_tcp_ao_counters(sk, &after_ao1, local_addr, -1, 100, 100))
			test_error("test_get_tcp_ao_counters()");
	}
	if (!check_restore) {
	...
	if (test_tcp_ao_counters_cmp(&before_ao1, &after_ao1, TEST_CNT_GOOD)) {
		close(sk);
		return;
	}
	if (different_keyids && test_tcp_ao_counters_cmp(&before_ao2, &after_ao2, TEST_CNT_GOOD)) {
		close(sk);
		return;
	}
