...
static void test_einval_del_key(void)
{
	...
	sk = prepare_defs(TCP_AO_DEL_KEY, &del);
	make_listen(sk);
	if (test_add_key(sk, "password", this_ip_dest, DEFAULT_TEST_PREFIX, 0, 0))
		test_error("add key");
	del.set_current = 1;
...
static void test_einval_ao_info(void)
{
	...
	sk = prepare_defs(TCP_AO_INFO, &info);
	info.ao_required = 1;
	if (test_set_md5(sk, tcp_md5_client, TEST_PREFIX, -1, "secret")) {
		if (errno != ENOPROTOOPT)
			test_error("setsockopt(TCP_MD5SIG_EXT)");
		test_skip("%s: setsockopt(TCP_MD5SIG_EXT) is not supported", "ao required with MD5 key");
		close(sk);
	} else {
		setsockopt_checked(sk, TCP_AO_INFO, &info, EKEYREJECTED, "ao required with MD5 key");
	}
