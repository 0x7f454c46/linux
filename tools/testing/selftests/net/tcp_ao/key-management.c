			   ...
			   fault_t inj)
{
	...
	test_fail("%s: can't delete a key %u:%u %d", tst_name, sndid, rcvid, err);
	return;
}
...
static void assert_no_current_rnext(const char *tst_msg, int sk)
{
	...
	if (ao_info.set_current || ao_info.set_rnext) {
		test_fail("%s: the socket has current/rnext keys: %d:%d",
			  tst_msg,
...
static void check_listen_socket(void)
{
	...
	try_delete_key("listen socket, delete current key from before listen()", sk, 100, 100, 0, -1, -1, 0);
	try_delete_key("listen socket, delete rnext key from before listen()", sk, 200, 200, 0, -1, -1, 0);
	close(sk);
...
const unsigned test_maclens[] = { 1, 4, 12, 16 };
#define MACLEN_SHIFT		2
#define ALGOS_SHIFT		4

static unsigned make_mask(unsigned shift, unsigned prev_shift)
{
	unsigned ret = BIT(shift) - 1;

	return ret << prev_shift;
}

static void init_key_in_collection(unsigned index, bool randomized)
{
	struct test_key *key = &collection.keys[index];
	unsigned algos_nr, algos_index;
...
static int init_default_key_collection(unsigned nr_keys, bool randomized)
{
...
static int test_add_key_cr(int sk, const char *pwd, unsigned pwd_len,
...
static int start_server(const char *tst_name, unsigned int port, size_t quota,
			...
			unsigned current_index, unsigned rnext_index)
{
...
static void server_rotations(const char *tst_name, unsigned int port,
			     size_t quota, unsigned rotations,
			     unsigned current_index, unsigned rnext_index)
{
	struct tcp_ao_counters tmp;
	unsigned i;
...
static int run_client(const char *tst_name, unsigned int port,
		      unsigned nr_keys, int current_index, int rnext_index,
...
static void end_client(const char *tst_name, int sk, unsigned nr_keys,
...
static void roll_over_keys(const char *tst_name, unsigned int port,
			   unsigned nr_keys, unsigned rotations,
			   unsigned current_index, unsigned rnext_index)
{
	struct tcp_ao_counters tmp;
	unsigned i;
