...
static void test_server_active_rst(unsigned int port)
{
	...
	if (test_add_key(lsk, "password", this_ip_dest, -1, 100, 100))
		test_error("setsockopt(TCP_AO_ADD_KEY)");
	if (test_get_tcp_ao_counters(lsk, &cnt1, this_ip_dest, -1, 100, 100))
	...
	if (test_get_tcp_ao_counters(lsk, &cnt2, this_ip_dest, -1, 100, 100))
		test_error("test_get_tcp_ao_counters()");
	...
	if (test_tcp_ao_counters_cmp(&cnt1, &cnt2, TEST_CNT_GOOD))
...
static void test_server_passive_rst(unsigned int port)
{
	...
	close(lsk);
	if (test_get_tcp_ao_counters(sk, &ao1, this_ip_dest, -1, 100, 100))
		test_error("test_get_tcp_ao_counters()");
...
static void test_client_active_rst(unsigned int port)
{
	...
	unsigned last = ARRAY_SIZE(sk) - 1;

	for (i = 0; i < ARRAY_SIZE(sk); i++) {
		sk[i] = socket(test_family, SOCK_STREAM, IPPROTO_TCP);
		if (sk[i] < 0)
			test_error("socket()");
		if (test_add_key(sk[i], "password", this_ip_dest, -1, 100, 100))
			test_error("setsockopt(TCP_AO_ADD_KEY)");
	}
...
static void test_client_passive_rst(unsigned int port)
{
	...
	if (test_add_repaired_key(sk, "password", 0, this_ip_dest, -1, 100, 100))
		test_error("setsockopt(TCP_AO_ADD_KEY)");
