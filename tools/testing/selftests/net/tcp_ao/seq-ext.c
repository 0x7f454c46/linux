			     ...
			     bool server)
{
	...
	ao_img->snd_sne_seq = new_seq2;
	ao_img->rcv_sne_seq = new_seq1;
}
			   ...
			   struct tcp_ao_counters *cnt)
{
	...
	if (test_add_repaired_key(sk, "password", 0, daddr, -1, 100, 100))
		test_error("setsockopt(TCP_AO_ADD_KEY)");
	test_ao_restore(sk, ao_img);

	if (test_get_tcp_ao_counters(sk, cnt, daddr, -1, 100, 100))
...
static void *server_fn(void *arg)
{
	...
	if (ao_img.snd_sne && ao_img.rcv_sne) {
		test_ok("SEQ extension incremented: %u/%u, %u/%u",
			ao_img.snd_sne, ao_img.snd_sne_seq,
			ao_img.rcv_sne, ao_img.rcv_sne_seq);
	} else {
		test_fail("SEQ extension was not incremented: %u/%u, %u/%u",
			ao_img.snd_sne, ao_img.snd_sne_seq,
			ao_img.rcv_sne, ao_img.rcv_sne_seq);
	}
