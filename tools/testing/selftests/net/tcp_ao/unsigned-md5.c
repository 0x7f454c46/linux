...
#define F_PREINSTALL_AO		3
#define F_PREINSTALL_MD5	4
#define F_POSTINSTALL		5
...
static int test_set_md5(int sk, const union tcp_addr in_addr,
			uint8_t prefix, int vrf)
{
...
static bool vrfs_enabled = true;
#define NEEDS_MD5	BIT(1)
#define NEEDS_VRF	BIT(2)

static bool should_skip_test(const char *tst_name, unsigned needs)
{
	if (needs & NEEDS_MD5 && !tcp_md5_enabled) {
		test_skip("%s: setsockopt(TCP_MD5SIG_EXT) is not supported", tst_name);
		return true;
	}
	if (needs & NEEDS_VRF && !vrfs_enabled) {
		test_skip("%s: VRF is not supported by kernel", tst_name);
		return true;
	}
	return false;
}
...
static void check_tcp_md5_support(void)
{
	...
	if (test_set_md5(sk, addr_any, 0, -1)) {
		       ...
		       unsigned needs, fault_t inj)
{
	...
	if (should_skip_test(tst_name, needs))
		return;

	lsk = test_listen_socket(this_ip_addr, port, 1);

	if (md5_addr && test_set_md5(lsk, *md5_addr, md5_prefix, -1))
		test_error("setsockopt(TCP_MD5SIG_EXT)");
	...
	if (ao_addr && test_get_tcp_ao_counters(lsk, &ao_cnt1, *ao_addr,
	...
	if (ao_addr && test_get_tcp_ao_counters(lsk, &ao_cnt2, *ao_addr,
						ao_prefix, sndid, rcvid))
		test_error("test_get_tcp_ao_counters()");
	close(lsk);
...
static void server_add_fail_tests(unsigned int *port)
{
	...
	try_accept("TCP-MD5 established: add TCP-AO key", (*port)++, &addr_any,
		   0, NULL, 0, 0, 0, 0, 0, NULL, 0, NEEDS_MD5, 0);
...
static void *server_fn(void *arg)
{
	...
	try_accept("MD5 server (INADDR_ANY): MD5 client", port++, &addr_any, 0,
		   NULL, 0, 0, 0, 0, 0, NULL, 0, NEEDS_MD5, 0);
	...
	try_accept("no sign server: AO client", port++, NULL, 0,
		   NULL, 0, 0, 0, 0, 0, "TCPAOKeyNotFound",
		   TEST_CNT_AO_KEY_NOT_FOUND, 0, F_TIMEOUT);
	...
	try_accept("AO+MD5 server: AO client (matching)", port++,
		   ...
		   100, 100, 0, "TCPAOGood", TEST_CNT_GOOD, NEEDS_MD5, 0);
...
static void try_connect(const char *tst_name, unsigned int port,
		       ...
		       union tcp_addr *ao_addr, uint8_t ao_prefix,
		       uint8_t sndid, uint8_t rcvid, uint8_t vrf,
		       fault_t inj, unsigned needs, union tcp_addr *bind_addr)
{
	time_t timeout;
	int sk, ret;
	...
	if (md5_addr && test_set_md5(sk, *md5_addr, md5_prefix, -1))
		test_error("setsockopt(TCP_MD5SIG_EXT)");
			  ...
			  fault_t inj, bool added_ao)
{
	...
	expected_to_fail = fault(F_PREINSTALL_AO) && added_ao;
	expected_to_fail |= fault(F_PREINSTALL_MD5) && !added_ao;
		    ...
		    fault_t inj)
{
	...
	if (strategy & PREINSTALL_MD5_FIRST) {
		if (test_set_md5(sk, md5_addr, md5_prefix, md5_vrf))
			test_error("setsockopt(TCP_MD5SIG_EXT)");
	}
	...
	if (strategy & PREINSTALL_MD5) {
		errno = 0;
		test_set_md5(sk, md5_addr, md5_prefix, md5_vrf);
...
static void client_add_fail_tests(unsigned int *port)
{
	try_to_add("TCP-AO established: add TCP-MD5 key",
		   ...
		   100, 100, NEEDS_MD5, F_POSTINSTALL);
...
static void client_vrf_tests(unsigned int *port)
{
	...
	try_to_preadd("VRF: TCP-AO key (no l3index) + TCP-MD5 key (no l3index)",
		      ...
		      NEEDS_MD5 | NEEDS_VRF, F_PREINSTALL_MD5);
	try_to_preadd("VRF: TCP-MD5 key (no l3index) + TCP-AO key (no l3index)",
		      (*port)++, PREINSTALL_MD5_FIRST | PREINSTALL_AO,
		      this_ip_addr, TEST_PREFIX, -1,
		      this_ip_addr, TEST_PREFIX, -1, 0, 100, 100,
		      NEEDS_MD5 | NEEDS_VRF, F_PREINSTALL_AO);
...
{
	test_init(67, server_fn, client_fn);
	return 0;
}
